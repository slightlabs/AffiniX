...
bool wheel_fits(std::uint64_t expiry_tick, std::uint64_t now_tick) {
    return expiry_tick <= now_tick || ((expiry_tick - now_tick) >> 32) == 0;
}
...
struct Model {
    ...
    int arm(TimePoint expiry, Duration period, RepeatMode mode, bool precise,
            int group, TimerFn&& fn) {
        ModelTimer mt;
        mt.tag = next_tag++;
        mt.expiry = expiry;
        mt.period = period;
        ...
        TimerGroup tg = group >= 0 && group < int(groups.size()) ? groups[group]
                                                                 : TimerGroup{};
        if (period.count() > 0)
            mt.tid =
                env->em.every(period, std::move(fn), expiry - now(), mode, tg);
        else if (precise)
        ...
        std::snprintf(
            ...
            (long long)period.count(), (int)precise, (int)mt.heap, group);
        note(mt.tag, b);
        return mt.tag;
    }
    ...
    void fire_due(TimePoint now, std::vector<int>& expected) {
        for (auto& [tag, mt] : timers) {
            ...
            std::uint64_t fire_tick =
                mt.heap ? now_tick() : tick_down(mt.effective);
            mt.heap = goes_to_heap(mt.expiry, mt.precise, fire_tick);
            mt.effective = mt.heap ? mt.expiry
                                   : TimePoint(Nanos(std::int64_t(
                                         tick_up(mt.expiry) * kTickNs)));
            char b[192];
...
std::vector<std::uint64_t> model_seeds() {
    ...
    if (const char* s = std::getenv("AFX_MODEL_SEEDS")) {
        ...
        for (const char* p = s;; ++p) {
            if (*p == ',' || *p == 0) {
                if (!cur.empty())
                    v.push_back(std::strtoull(cur.c_str(), nullptr, 0));
                cur.clear();
                if (*p == 0) break;
            } else
                cur += *p;
...
TEST_CASE("model: timers match a naive reference under VirtualClock") {
    for (std::uint64_t seed : model_seeds()) {
        ...
        auto rand_delay = [&]() -> Duration {
            switch (rng() % 10) {
                case 0:
                    return Nanos(rng() % 900);  // sub-tick
                case 1:
                    return Nanos(kTickNs) * (256 * 256 + rng() % 512);  // L2
                case 2:
                    return Duration(24h) +
                           Nanos(rng() % 1000);  // >horizon->heap
                default:
                    return Nanos(kTickNs) * (1 + rng() % 3000);
            }
        };
        ...
        for (int step = 0; step < kSteps; ++step) {
            ...
            if (op < 35) {  // arm one-shot
                ...
                m.arm(m.now() + rand_delay(), Duration{}, RepeatMode::FixedRate,
                      false, g, make_fn(m.next_tag));
            } else if (op < 45) {  // arm precise at()
                m.arm(m.now() + rand_delay() - Duration(1ms), Duration{},
                      RepeatMode::FixedRate, true, -1, make_fn(m.next_tag));
            } else if (op < 60) {  // arm repeating
                Duration p = Nanos(kTickNs) * (1 + rng() % 500);
                auto mode =
                    rng() % 2 ? RepeatMode::FixedRate : RepeatMode::FixedDelay;
                ...
                if (!m.live_tags.empty()) {
                    ...
                    mt.heap =
                        m.goes_to_heap(mt.expiry, mt.precise, m.now_tick());
                ...
                switch (rng() % 10) {
                    case 0:
                        jump = Nanos(kTickNs) * (256 + rng() % 3840);
                        break;
                    case 1:
                        jump = Nanos(kTickNs) * (65536 + rng() % 65536);
                        break;
                    default:
                        jump = Nanos(rng() % (kTickNs * 64));
                        break;
                }
                        ...
                        if (it != fired_ctx.end()) {
                            ...
                            std::snprintf(
                                ...
                                (long long)it->second.scheduled
                                    .time_since_epoch()
                                    .count(),
                                (long long)it->second.now.time_since_epoch()
                                    .count(),
                                it->second.missed);
                            ctx_str = b;
                        }
                        MESSAGE(
                            "fired-not-expected tag=", t,
                            " expiry=", mt.expiry.time_since_epoch().count(),
                            " eff=", mt.effective.time_since_epoch().count(),
                            " period=", mt.period.count(),
                            " precise=", mt.precise, " heap=", mt.heap,
                ...
                for (int tag : fired) {
                    ...
                    CHECK((tc.scheduled == mt.expiry || mt.period.count() > 0));
                    (void)mt;
                }
            }
            m.sweep_dead();
