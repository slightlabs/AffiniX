...
template <class T> struct ResultStorage {
    union U {
        ...
        U& operator=(const U& o) { err = o.err; return *this; }
    } u;
    bool has = false;

    ResultStorage() = default;
    ~ResultStorage() { if (has) u.val.~T(); }
    ResultStorage(const ResultStorage& o) : has(o.has) {
        if (has) new (&u.val) T(o.u.val); else u.err = o.u.err;
    }
    ResultStorage(ResultStorage&& o) noexcept(std::is_nothrow_move_constructible_v<T>)
        : has(o.has) {
        if (has) new (&u.val) T(std::move(o.u.val)); else u.err = o.u.err;
    }
    ...
    ResultStorage& operator=(ResultStorage&& o) noexcept(std::is_nothrow_move_constructible_v<T>) {
        if (this == &o) return *this;
        if (has) u.val.~T();
        has = o.has;
        if (has) new (&u.val) T(std::move(o.u.val)); else u.err = o.u.err;
...
class Result {
public:
    Result() : s_{} { s_.has = true; new (&s_.u.val) T(); }
    Result(const T& v) { s_.has = true; new (&s_.u.val) T(v); }
    Result(T&& v) { s_.has = true; new (&s_.u.val) T(std::move(v)); }
    ...
    T& value() & noexcept { assert(s_.has); return s_.u.val; }
    const T& value() const& noexcept { assert(s_.has); return s_.u.val; }
    T&& value() && noexcept { assert(s_.has); return std::move(s_.u.val); }
    T& operator*() & noexcept { return value(); }
    const T& operator*() const& noexcept { return value(); }
    T* operator->() noexcept { return &value(); }
    const T* operator->() const noexcept { return &value(); }

    Error error() const noexcept { assert(!s_.has); return s_.u.err; }
...
public:
    ...
    Error error() const noexcept { assert(!err_.ok()); return err_; }
