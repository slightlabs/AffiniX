...
class FlightRecorder {
  public:
    ...
    void record(EventKind k, std::uint32_t handle, std::uint32_t a = 0,
                std::uint32_t b = 0, std::uint64_t trace = 0) noexcept {
#ifdef AFX_FLIGHT_RECORDER
        auto i =
            head_.fetch_add(1, std::memory_order_relaxed) & (kCapacity - 1);
